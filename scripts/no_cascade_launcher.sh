...
do
    ...
    echo "sbatch /home/ucl/elen/nsayez/bio-blueprints/scripts/no_cascade.sh $random_move $focus_on_group "$use_dtypes" $use_npy "$use_classes" $max_epochs "$parts_to_train" $lr $T_max $eta_min $batch_size $num_workers $c $job_name $wandb_project "$visual_input" "$numeric_input" $cascade"
    # sbatch /home/ucl/elen/nsayez/bio-blueprints/scripts/no_cascade.sh $random_move $focus_on_group "$use_dtypes" $use_npy "$use_classes" $max_epochs "$parts_to_train" $lr $T_max $eta_min $batch_size $num_workers $c $job_name $wandb_project "$visual_input" "$numeric_input" $cascade
done



# sbatch /home/ucl/elen/nsayez/bio-blueprints/scripts/no_cascade.sh $random_move $focus_on_group "$use_dtypes" $use_npy "$use_classes" $max_epochs "$parts_to_train" $lr $T_max $eta_min $batch_size $num_workers $char_to_balance
