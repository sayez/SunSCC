...
output_dir=param_optimization_sunscc
...
for i in ${kernel_bandwidthLon[@]}; do
    for j in ${kernel_bandwidthLat[@]}; do
        echo "sbatch --cpus-per-task=$num_cpu ./scripts/clustering_compute_huge_dicts.sh  $input_type $look_distance $i $j  $n_iterations $num_cpu \
        $wl_dir $masks_dir $sqlite_path $root_dir $output_dir"
        sbatch --cpus-per-task=$num_cpu ./scripts/clustering_compute_huge_dicts.sh  $input_type $look_distance $i $j  $n_iterations $num_cpu \
        $wl_dir $masks_dir $sqlite_path $root_dir $output_dir
    done
done
